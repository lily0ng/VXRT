...
import { Database, Plus, Upload, Download, Layers } from 'lucide-react';
  ...
  return (
    ...
    >
      <motion.div variants={itemVariants} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-heading font-bold text-ghost-white">Container Registry</h1>
          <p className="text-muted-gray mt-1">Private Docker registry for images</p>
        </div>
      ...
      <motion.div variants={itemVariants} className="bg-dark-base border border-steel-gray rounded-xl p-12 text-center">
        <Database className="w-16 h-16 text-exploit-red/30 mx-auto mb-4" />
        <h3 className="text-xl font-semibold text-ghost-white mb-2">No Repositories</h3>
        <p className="text-muted-gray max-w-md mx-auto">
          Create repositories to store and manage container images
        </p>
      </motion.div>
    </motion.div>
  );
