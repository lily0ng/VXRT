...
import { CheckCircle, Plus, FileText, Download } from 'lucide-react';
  ...
  return (
    ...
    >
      <motion.div variants={itemVariants} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-heading font-bold text-ghost-white">Compliance Reports</h1>
          <p className="text-muted-gray mt-1">Generate compliance documentation</p>
        </div>
      ...
      <motion.div variants={itemVariants} className="bg-dark-base border border-steel-gray rounded-xl p-12 text-center">
        <CheckCircle className="w-16 h-16 text-exploit-red/30 mx-auto mb-4" />
        <h3 className="text-xl font-semibold text-ghost-white mb-2">No Compliance Reports</h3>
        <p className="text-muted-gray max-w-md mx-auto">
          Generate reports for SOC2, PCI-DSS, HIPAA, and more
        </p>
      </motion.div>
    </motion.div>
  );
